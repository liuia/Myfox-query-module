...
//				console.log(d);
		...
		request.write(encodeURIComponent(get.readCache + sep + get.writeCache + sep
				+ get.isDebug + sep + get.explain + "\r\nsqlMode\r\n" + get.sql
				+ "\r\n"));
		request.end();
	});
...
function checkPhrase(str) {
	console.log("checkPhrase:" + hash.md5(str));
...
function changeLogLevel(action, level, res) {
	masterLogger.notice("getMonitorOrder-changeLogLevel", "action:" + action
			+ "|level:" + parseInt(level) + "|token:" + token);
	var data = {
		id : token,
		type : TYPE.CONTROL,
...
function cleanCache(which, res) {
	console.log("in cleanCache" + which);

	if (/^sqlcache/i.test(which)) {
		var get = which.split(":");
		masterLogger.notice("getMonitorOrder-cleanCache",
				"clean sqlcache|keyVersion:" + get[1]);
	...
	masterLogger.notice("getMonitorOrder-cleanCache", "which:" + which
			+ "|token:" + token);
	send(data);
}
/* }}} */
...
function send(data) {
	...
	ui.sendto(masterConf.port, data);
}
/* }}} */
