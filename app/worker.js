...
var msg_handle = function(data) {
	sys.puts("worker:msg_handle: " + data.type);
	...
	worker.sendto(masterConf.userPort, result);
};
/* }}} */
...
var post_handle = function(req, res) {
	...
	req.on('end', function() {
		...
		query.req = req;
		query.res = res;
		workerLogger.notice('getQuery|token:' + query.token + '[' + query.ip + '('
				+ req.appIp + ')' + ']', JSON.stringify(parseData));
		if (!REQUEST_QUEUE.push(query)) { /* 若queryQueue中不存在，插入queryQueue，并且执行后续请求动作;若存在则到此为止 * */
			query.getData(function(query) {
				REQUEST_QUEUE.end(query, function(query) {
					...
					workerLogger.notice('RES|token:' + query.token, JSON.stringify({
						timeUse : Date.now() - query.start,
...
function updateTrend() {
	...
	trend.push({
		d : Date.now(),
		queryNum : __STAT__.totalReqs - last_count
	});
