...
var hash       = require(__dirname + '/rules/hashes')
var mirror     = require(__dirname + '/rules/mirror');
var numsplit   = require(__dirname + '/rules/numsplit');
var LCache     = require(__dirname + '/cache/lcache');
...
function findNodes(reqObj,table,field,cb){
  ...
  if(get !== false){
    ...
    var sql = "SELECT hosts_list,real_table,modtime,unique_key FROM $$v1 WHERE route_sign = $$v2 AND table_name = $$v3 AND route_text=$$v4 AND route_flag >= 300 AND route_flag < 400";
